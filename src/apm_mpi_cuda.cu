...
int main(int argc, char ** argv) {
   ...
   *       ******/
    ...
    for ( i = 0 ; i < nb_patterns ; i++ ) {
        ...
        char * d_pattern;
        int * d_result;
        int n_max = (scounts[rank]-size_pattern + 1);
        int * result;
	result=(int *) malloc(sizeof(int)*n_max);
	cudaMalloc((void **)&d_rcv_buf, scounts[rank]*sizeof(char));
	cudaMalloc((void **)&d_pattern, size_pattern*sizeof(char));
	cudaMalloc((void **)&d_result, n_max*sizeof(int));

	cudaMemcpy(d_rcv_buf, rcv_buf, scounts[rank]*sizeof(char), cudaMemcpyHostToDevice);
